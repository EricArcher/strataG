...
Most functions in _strataG_ operate on genetic data provided as a _gtypes_ object. _gtypes_ contain information about individual genotypes, ploidy, sequences, stratification, and can carry auxiliary information about individuals. Internally, _gtypes_ are stored as R S4 classes, for which accessor functions are available for all of the slots in the class.

There are multiple ways to create a _gtypes_ object. A common format is to have the genetic data in a _data.frame_ or _matrix_ in which each row is a genotype for an individual and the columns represent the alleles at each locus. A function, _readGenData_, has been provided as a wrapper for _read.csv_ that will read a standard comma-delimited text file into a _data.frame_ while recognizing commonly used missing data codes. 

For each locus, there should be a column for each allele. Each column name should contain the locus name and a suffix that designates the allele. For example, the two columns of a diploid locus named "ABC" would be named "ABC.1" and "ABC.2" respectively. If all alleles are in a single column, the _alleleSplit_ function will create a matrix with alleles split into multiple columns.
