...
The R programming language has rapidly become the platform of choice for population genetics.

The _strataG_ pacakge is designed to be an extensible toolkit for population genetics. It incorporates functions to generate standard summary statistics, conduct analyses of population structure, 

## Data Import

Most functions in _strataG_ operate on genetic data provided as a _gtypes_ object, which contain information about individual genotypes, ploidy, sequences, stratification, and can carry auxiliary information as well. _gtypes_ are stored as R S4 classes and accessor functions are provided for all of the slots in the class.

There are multiple ways to create a _gtypes_ object. A standard inport format is to have the genetic data in a _data.frame_ or _matrix_, where each row is a genotype for an individual and columns represent the alleles at each locus. The _readGenData_ function in _strataG_ is a wrapper for _read.csv_ that will read a standard comma-delimited text file into a _data.frame_ recognizing commonly used missing data codes. 
