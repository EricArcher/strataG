---
title: 'strataG: An R package for genetic data summaries and analyses of population structure'
author: "F.I. Archer"
date: "`r Sys.Date()`"
...
The R programming language has rapidly become a popular platform for population genetics analyses. Several packages have been developed to efficiently store and manipulate genetic data, summarize genetic diversity, and conduct phylogenetic analyses to name a few. 

The _strataG_ pacakge is designed to be an extensible toolkit for population genetics. It incorporates functions to generate standard summary statistics, conduct analyses of population structure, and run several popular external programs such as Genepop and STRUCTURE.

## Data Import

Most functions in _strataG_ operate on genetic data provided as a `gtypes` object. `gtypes` contain information about individual genotypes, ploidy, sequences, stratification, and can carry auxiliary information about individuals. Internally, `gtypes` are stored as R S4 classes, for which accessor functions are available for all of the slots in the class.

There are multiple ways to create a `gtypes` object. A common format starts with the genetic data in a `data.frame` or `matrix` in which each row is a genotype for an individual and the columns represent the alleles at each locus. A function, `readGenData`, has been provided as a wrapper for `read.csv` that will read a standard comma-delimited text file into a `data.frame` while recognizing commonly used missing data codes. 

For each locus, there should be a column for each allele. Each column name should contain the locus name and a suffix that designates the allele. For example, the two columns of a diploid locus named "ABC" would be named "ABC.1" and "ABC.2" respectively. If all alleles are in a single column, the `alleleSplit` function will create a matrix with alleles split into multiple columns.
 
### Creating _gtypes_ with the standard _new_ function
...
A _gtypes_ object is an R S4 class with the following slots:

* __\@loci__ a data.frame containing the allelic data as one column per locus. Alleles are on multiple rows per column with samples listed in the same order for each allele. rownames are sample names plus allele number formatted as 12345.1 and 12345.2 where 12345 is the sample name and 1 and 2 are the first and second alleles. colnames are unique locus names.
...
A _gtypes_ object can be subset using the standard R '[' indexing operation. The first argument specifies the desired individuals, the second is the loci to return, and the third is the strata. All standard R indexing operations involving numerical, character, or logical vectors work for each argument.
...
You can specify the stratification scheme when creating a _gtpyes_ object as in the examples above. Once a _gtypes_ object has been created, you can also change the stratification scheme by either supplying a new vector for the \@strata slot:
```{r}
