...
// for now calcfst has been added to the bottom of the file to keep from having to call the function in R... however when these are all complied together
// you should be able to remove the function from the bottom 
// since you have to call calcfst twice the time will be at least twice as long as calcfst 


/*** R
calc.fst.primeC <- function(g) {
  g <- check.gtypes(g, "diploid")
  est <- createnewg(g$locus.data, g$strata)

  result <- list(stat.name = "F'st", estimate = est)
...
// [[Rcpp::export]]
double createnewg(NumericMatrix locusdata, NumericVector strata){
  // function declaration
  double calcfst(NumericMatrix, NumericVector);

  int nrow = locusdata.nrow(), ncol = locusdata.ncol();
  
  NumericMatrix newlocusdata(nrow, ncol);
  for(int i = 0; i < nrow; i++){
    for(int j = 0; j< ncol; j++){
      if(locusdata(i,j) == -1){
        ...
        newlocusdata(i,j) = strata(i) + (locusdata(i,j)/100);
      }
    }
  }
  
  double fst = calcfst(locusdata, strata);
  double fstmax = calcfst(newlocusdata, strata);
  double fstprime = fst/fstmax;
