...
#'   permutation test.
#' @param stats a character vector specifying which anlayses to conduct.
#'   Possible choices are: "phist", "fst", "fst.prime", "gst", "gst.prime", 
#'   "gst.dbl.prime", "d", or "chi2", or "all".
#' @param stat.list a list of popStructStat functions. 
#'    Generated by \code{statList}.
...
popStructTest <- function(g, nrep = 100, stats = "all", type = "both", 
  ...
  if(type %in% c("both", "overall")) {
    overall <- overallTest(g = g, nrep = nrep, stat.list = stat.list, 
  ...
  if(type %in% c("both", "pairwise") & nStrata(g) > 2) {
    pairwise <- pairwiseTest(g = g, nrep = nrep, stat.list = stat.list, 
      keep.null = keep.null, num.cores = num.cores, quietly = quietly, ...
    )
  }
...
#' @export
#' 
overallTest <- function(g, nrep = 100, stat.list = statList("all"), 
