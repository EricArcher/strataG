#' @title Tajima's D
#' @description Calculate Tajima's D for a set of sequences to test for selection.
#' 
...
tajimasD <- function(x) {
#   x <- if(is.character(x)) {
#     if(length(x) != 1) stop("If 'x' is a character, it cannot have more than one element.")
#     read.fasta(x)
#   } else if(is.gtypes(x, show.warnings = FALSE)) {
#     if(!is.haploid(x)) stop("gtypes object must be haploid to calculate Tajima's D")
#     if(is.null(x$sequences)) stop("Sequences are NULL, Tajima's D cannot be calculated")
#     x$sequences
#   } else if(is.aligned.seq(x)) {
#     x
#   } else stop("'x' is not a filename, list of sequences or 'gtypes' object.")
#   
#   num.sites <- length(x[[1]]) 
#   #calculate mean pairwise difference   
#   mean.pairwise.diff <- function(dna.seq){   
#      pairwise.diff <- dist.dna(as.DNAbin(dna.seq), model = "N", pairwise.deletion = TRUE, as.matrix = TRUE)
#      pairwise.diff <- pairwise.diff[lower.tri(pairwise.diff)]
#      mean(pairwise.diff)
#   }
#      
#   #calculate number of segregating sites
#   calc.num.seg.sites <- function(dna.seq){   
#      df.seqs <- as.data.frame(dna.seq)
#      seg.sites <- apply(df.seqs, 1, unique)
#      sum(sapply(1:length(seg.sites), function(i) {
#        ifelse(length(seg.sites[[i]]) > 1, 1, 0)
#      }))
#   }
#   
#   pi <- mean.pairwise.diff(x)
#   S <- calc.num.seg.sites(x) # number segregating sites
#   n <- length(x) # number individuals
#   
...
#   beta.D <- function(tajima.D, alpha = Alpha, beta = Beta, Dmin = DMin, Dmax = DMax) { # give P(D, given the other 4)
#     gamma(alpha + beta)*(Dmax - tajima.D)^(alpha - 1)*(tajima.D - Dmin)^(beta - 1)/
...
#     to <- DMax
#   } 
#   prob <- integrate(beta.D, from, to, alpha = Alpha, beta = Beta, Dmin = DMin, Dmax = DMax)
#   
