...
fscWrite <- function(
  ...
  cols <- c("pop.size", "sample.size")
  pop.info[, cols] <- pop.info[, cols] * ploidy
  
  if(is.null(label)) label <- "fastsimcoal.output"
  label <- make.names(label)
  file <- paste(label, ".par", sep = "")
  mig.rates <- if(!is.null(mig.rates)) {
    if(is.list(mig.rates)) mig.rates else list(mig.rates)
  }
  ...
  for(i in 1:nrow(pop.info)) {
    write(
      pop.info[i, c("sample.size", "sample.times")], 
      file = file, 
      append = T
    )
  }
  ...
  for(i in 1:nrow(pop.info)) {
    write(as.character(pop.info[i, "growth.rate"]), file, append = T)
  }
  ...
  if(!is.null(mig.rates)) {
    for(i in 1:length(mig.rates)) {
      ...
      for(r in 1:nrow(mig.rates[[i]])) {
        write(
          paste(as.character(mig.rates[[i]][r, ]), collapse = " "), 
          file = file, 
          append = T
        )
      }
...
fscRead <- function(file, locus.params, label.haplotypes = TRUE) {
  .formatGenotypes <- function(x, ploidy) {
    ...
    # rename loci
    locus_names <- paste("Locus", swfscMisc::zero.pad(1:nloci), sep = "_")
  ...
  switch(
    ...
    DNA = { # diploid SNPs
      ...
      if(attr(locus.params, "ploidy") == 2) {
        ...
        if(label.haplotypes) labelHaplotypes(g) else g
      }
    },
    MICROSAT = {
