...
#' @param locus.type a character representation of what type of marker to
#'   simulate. Can be "dna", "msat", "snp", or "standard".
...
fscLocusParams <- function(locus.type = c("dna", "msat", "snp", "standard"),
  ...
  df <- switch(
    ...
    standard = createLocusParams(
      chromosome, "STANDARD", num.loci, recomb.rate, mut.rate, NA, NA, 2, 
      num.chrom
    )
  )
...
#' @param prop.migrants Expected proportion of migrants to move from source to
#'   sink.
...
#' @param new.mig.mat Number of new migration matrix to be used further back in
#'   time. The matrices are provided in the \code{mig.rates} list argument in
#'   \code{\link{fastsimcoal}}. The first matrix has index 0 in the vector
#'   provided to this argument. See the fastsimcoal manual for more information.
#' 
#' @rdname fastsimcoal.input
#' @export
#' 
